...
class FCPTabBarTemplate {
  ...
  init(obj: [String: Any]) {
    ...
    self.objcTemplates = FCPTabBarTemplate.parseTemplates(obj["templates"] as! [[String: Any]])
    self.templates = self.objcTemplates.map { $0.get }
  }

  private static func parseTemplates(_ templatesData: [[String: Any]]) -> [FCPTabBarChildTemplate] {
  ...
  public func updateTemplates(newTemplatesData: [[String: Any]]) {
    let existingTemplatesById: [String: FCPTabBarChildTemplate] = Dictionary(
    ...
    self.objcTemplates = newTemplates.map { template in
      if let existing = existingTemplatesById[template.elementId] {
        ...
        if let existingList = existing as? FCPListTemplate,
          let newList = template as? FCPListTemplate
        {
